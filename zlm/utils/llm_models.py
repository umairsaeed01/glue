...
# import openai  # REMOVE this line, not needed with new client interface
import pandas as pd
...
class ChatGPT:
    def __init__(self, api_key, model, system_prompt):
        import os # Make sure os is imported for debugging
        if system_prompt.strip():
            self.system_prompt = {"role": "system", "content": system_prompt}

        # --- Debugging ---
        print(f"DEBUG: api_key received by ChatGPT.__init__: {api_key}")
        env_api_key = os.environ.get("OPENAI_API_KEY")
        print(f"DEBUG: os.environ.get('OPENAI_API_KEY') inside ChatGPT.__init__: {env_api_key}")
        # --- End Debugging ---

        # Explicitly pass None if api_key is missing/empty to trigger env var check by OpenAI library
        effective_api_key = api_key if api_key and api_key.strip() else None
        print(f"DEBUG: effective_api_key passed to OpenAI(): {effective_api_key}") # Debug print
